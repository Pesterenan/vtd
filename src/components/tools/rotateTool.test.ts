...
describe('RotateTool', () => {
  ...
  it('should start rotating on mouse down', () => {
    ...
    expect(requestSpy).toHaveBeenCalledWith('workarea:adjustForCanvas', { position: { x: 10, y: 20 } });
    expect((rotateTool as any).isRotating).toBe(true);
    expect((rotateTool as any).startPosition).toEqual({ x: 10, y: 20 });
    expect(emitSpy).toHaveBeenCalledWith('workarea:update');
  });
  ...
  it('should update rotation on mouse move', () => {
    ...
    expect(requestSpy).toHaveBeenCalledWith('workarea:adjustForCanvas', { position: { x: 10, y: 10 } });
    expect(emitSpy).toHaveBeenCalledWith('transformBox:updateRotation', { delta: 45 });
  });
  ...
  it('should reset on mouse up', () => {
    ...
    expect((rotateTool as any).isRotating).toBe(false);
    expect((rotateTool as any).startPosition).toBeNull();
  });
});
...
describe('ScaleTool', () => {
  ...
  it('should start scaling on mouse down', () => {
    ...
    expect(requestSpy).toHaveBeenCalledWith('workarea:adjustForCanvas', { position: { x: 10, y: 20 } });
    expect((scaleTool as any).isScaling).toBe(true);
    expect((scaleTool as any).startPosition).toEqual({ x: 10, y: 20 });
    expect(emitSpy).toHaveBeenCalledWith('workarea:update');
  });

  it('should start proportional scaling on shift-click', () => {
    ...
    expect((scaleTool as any).isProportional).toBe(true);
  });
  ...
  it('should reset on mouse up', () => {
    ...
    expect((scaleTool as any).isScaling).toBe(false);
    expect((scaleTool as any).startPosition).toBeNull();
    expect((scaleTool as any).isProportional).toBe(false);
  });
});